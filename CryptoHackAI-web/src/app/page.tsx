...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: elizaMessage },
      ]);
    } catch (error) {
      ...
      <div className="shadow-md rounded p-8 w-full max-w-3xl bg-white">
        ...
        <div className="p-4 overflow-y-auto h-96 bg-customGreen/50 rounded-lg">
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`mb-2 ${msg.role === 'assistant' ? 'text-left text-blue-800' : 'text-right text-green-800'}`}
            >
              <p
                className={`inline-block px-4 py-2 rounded ${msg.role === 'assistant' ? 'bg-blue-200' : 'bg-green-200'}`}
              >
                {msg.content}
              </p>
            </div>
